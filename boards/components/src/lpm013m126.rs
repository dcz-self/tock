...
//!     }
//!     .finalize(
//!         components::lpm013m126_component_helper!(nrf52::rtc::Rtc<'static>),
//!     );
//! display.initialize();
...
macro_rules! lpm013m126_component_helper {
    ($A:ty) => {{
        ...
        static mut BUFFER: [u8; BUFFER_SIZE] = [0; lpm013m126::BUFFER_SIZE];

        let lpm013m126 = static_buf!(
            Lpm013m126<'static, VirtualMuxAlarm<'static, $A>>
        );
        let alarm = static_buf!(VirtualMuxAlarm<'static, $A>);
        (&mut alarm, &mut BUFFER, &mut lpm013m126)
    }};
}
